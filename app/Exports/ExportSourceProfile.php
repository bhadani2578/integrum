...
class ExportSourceProfile implements FromCollection, WithHeadings
{
    protected $data;
    ...
    public function collection()
    {
        try {
            ...
            if ($this->data['granularity_level'] == 1) {
                $data = AnnualSourceData::where('profile_id', $this->data['profile_id'])->get();
                ...
                $data = MonthlySourceData::where('profile_id', $this->data['profile_id'])
            ...
            elseif ($this->data['granularity_level'] == 3) {
                $data = TODStateSourceData::where('profile_id', $this->data['profile_id'])->select('slot', 'jan', 'feb', 'mar','apr','may','jun','jul','aug','sep','oct','nov','dec','consumed_unit')->get();
                ...
                $consumption_tod = SourceTod::where('profile_id', $this->data['profile_id'])->get();
                ...
                $data = WeeklySourceData::where('profile_id', $this->data['profile_id'])
                ...
                foreach ($data as $key => $value) {
                    ...
                    $consumedUnitPerInterval = ($value['consumed_unit'] / 7) / (24 * $slotPerHour);
                        ...
                        for ($hour = 0; $hour < 24; $hour++) {
                            ...
                            for ($slot = 0; $slot < $slotPerHour; $slot++) {
                                ...
                                $interval = [
                                    ...
                                    'Consumption Unit' => round($consumedUnitPerInterval)
                                ];
                ...
                $data = HourlySourceData::where('profile_id', $this->data['profile_id'])->select('hours', 'consumed_unit')->get();
                foreach ($data as $key => $value) {
